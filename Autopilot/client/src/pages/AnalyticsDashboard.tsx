...
export default function AnalyticsDashboard() {
    ...
    const [deployedFundsSortOrder, setDeployedFundsSortOrder] = useState<'asc' | 'desc' | null>(null);
    const [totalQtySortOrder, setTotalQtySortOrder] = useState<'asc' | 'desc' | null>(null);

    // Derived state for filtering and sorting
    const filteredUsers = allUserVolumes
        .filter(user =>
    ...
    const handleSortToggle = () => {
        ...
        // Reset other sorts when this one is activated
        setDeployedFundsSortOrder(null);
        setTotalQtySortOrder(null);
    };

    const handleDeployedFundsSortToggle = () => {
        setDeployedFundsSortOrder(current => {
            if (current === null) return 'desc';
            if (current === 'desc') return 'asc';
            return null;
        });
    ...
    const handleTotalQtySortToggle = () => {
        setTotalQtySortOrder(current => {
            if (current === null) return 'desc';
            if (current === 'desc') return 'asc';
            return null;
        });
    ...
    // Fetch total volumes generated by users
    useEffect(() => {
                                                        ...
                                                        <TableHead className="text-start">
                                                            <div className="flex items-center">
                                                                ...
                                                                <Button
                                                                    variant="ghost"
                                                                    size="sm"
                                                                    onClick={handleDeployedFundsSortToggle}
                                                                    className="h-6 w-6 p-0 hover:bg-muted"
                                                                >
                                                                    {deployedFundsSortOrder === null && <ArrowUpDown className="h-3 w-3" />}
                                                                    {deployedFundsSortOrder === 'desc' && <ArrowDown className="h-3 w-3" />}
                                                                    {deployedFundsSortOrder === 'asc' && <ArrowUp className="h-3 w-3" />}
                                                                </Button>
                                                            </div>
                                                        </TableHead>
                                                        <TableHead className="text-start">
                                                            <div className="flex items-center">
                                                                ...
                                                                <Button
                                                                    variant="ghost"
                                                                    size="sm"
                                                                    onClick={handleTotalQtySortToggle}
                                                                    className="h-6 w-6 p-0 hover:bg-muted"
                                                                >
                                                                    {totalQtySortOrder === null && <ArrowUpDown className="h-3 w-3" />}
                                                                    {totalQtySortOrder === 'desc' && <ArrowDown className="h-3 w-3" />}
                                                                    {totalQtySortOrder === 'asc' && <ArrowUp className="h-3 w-3" />}
                                                                </Button>
                                                            </div>
                                                        </TableHead>
                                                        <TableHead className="text-start">
                                                            <div className="flex items-center">
                                                                <span>Total Volumes Generated</span>
                                                                <Button
                                                                    variant="ghost"
                                                                    size="sm"
