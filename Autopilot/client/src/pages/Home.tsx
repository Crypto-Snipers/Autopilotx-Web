...
const {
  ...
  queryFn: async () => {
    ...
    const res = await apiRequest<number>(
      "GET",
      `/api/futures-wallet?email=${encodeURIComponent(email)}`
    );
  ...
  const handleRefreshBalance = async () => {
    ...
    try {
      if (!email) throw new Error('Email is required');
      await apiRequest("GET", `/api/user/refreshed_wallet?email=${encodeURIComponent(email)}`);
    } catch (error) {
      console.error('Failed to refresh balance:', error);
    } finally {
      const elapsed = Date.now() - start;
      const delay = Math.max(300 - elapsed, 0);
      timeoutId = setTimeout(() => setIsManuallyRefreshing(false), delay);
    }
  ...
  if (typeof futureWallet === "number" && !isNaN(futureWallet)) {
    ...
    // DELTA → SHOW USD ONLY
    if (brokerName === "delta_exchange") {
      setBalances({
        usd: futureWallet,  // store in USD
        inr: 0,
      });
      setCurrencies(["usd"]);
      ...
      sessionStorage.setItem(
        "balances",
        JSON.stringify({ usd: futureWallet, inr: 0 })
      );
      sessionStorage.setItem("currencies", JSON.stringify(["usd"]));
    ...
    sessionStorage.setItem(
      "balances",
      JSON.stringify({ usd: 0, inr: futureWallet })
    );
  ...
  // FALLBACK
  if (isBalanceError || isLoadingBalance) {
    const storedBalances = sessionStorage.getItem("balances");
    const storedCurrencies = sessionStorage.getItem("currencies");
    ...
    if (storedBalances && storedCurrencies) {
      try {
        ...
        console.error("Error parsing stored balance:", e);
      }
    }
  }
        ...
        <main className="p-2 md:p-4">
          <div className="flex justify-between w-full min-h-0">
            ...
            <div ref={scrollRef} className="basis-[70%] flex flex-col min-h-0 max-h-[calc(100vh-140px)] overflow-y-auto snap-y snap-mandatory scroll-smooth pr-4 custom-scrollbar">
              {brokerIsActive === "true" ? (
                <div className="border border-2 rounded-lg flex items-center justify-center">
                                            ...
                                            <div className="w-32 h-10 rounded-lg flex items-center justify-start overflow-hidden">
                                              {brokerName === 'delta_exchange' ? (
                                                ...
                                                <span className="font-bold text-[25px] select-none mr-3 whitespace-nowrap">
                                                  <span className="text-[#2c4166ff] dark:text-white" style={{ fontFamily: 'Poppins, Arial, sans-serif', letterSpacing: '-0.04em' }}>Coin</span>
